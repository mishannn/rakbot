...
bool DataBlockEncryptor::Decrypt(unsigned char *input, int inputLength, unsigned char *output, int *outputLength) {
	...
	memcpy((char*)& encodedPad, input + sizeof(char) + sizeof(checkSum), sizeof(encodedPad));
	...
	// Get the data length
	*outputLength = inputLength - sizeof(char) - sizeof(checkSum) - sizeof(encodedPad) - paddingBytes;

	// Calculate the checksum on the data.
	checkSumCalculator.Add(input + sizeof(checkSum), *outputLength + sizeof(char) + sizeof(encodedPad) + paddingBytes);
	...
	// Read the data
	if (input == output)
		memmove(output, input + sizeof(char) + sizeof(checkSum) + sizeof(encodedPad) + paddingBytes, *outputLength);
	else
		memcpy(output, input + sizeof(char) + sizeof(checkSum) + sizeof(encodedPad) + paddingBytes, *outputLength);
