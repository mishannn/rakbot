...
void AsynchronousFileIO::IncreaseUserCount()
{
	...
	if ( userCount == 1 )
	{
		...
		for ( uint32_t i = 0; i < systemInfo.dwNumberOfProcessors * 2; i++ )
...
bool AsynchronousFileIO::AssociateSocketWithCompletionPort( SOCKET socket, uint32_t dwCompletionKey )
{
...
BOOL ReadAsynch( HANDLE handle, ExtendedOverlappedStruct *extended )
{
	...
	if ( !success )
	{
		uint32_t dwErrCode = GetLastError();
...
unsigned __stdcall ThreadPoolFunc( LPVOID arguments )
{
	...
	while ( 1 )
	{
		...
		BOOL returnValue = GetQueuedCompletionStatus(
			completionPort,
			&dwIoSize,
			( LPuint32_t ) & lpClientContext,
			&temp, INFINITE );

		lpOverlapped = ( ExtendedOverlappedStruct* ) temp;

		uint32_t dwIOError = GetLastError();
