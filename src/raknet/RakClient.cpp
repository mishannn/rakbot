...
bool RakClient::Connect(const char* host, unsigned short serverPort, unsigned short clientPort, unsigned int depreciated, int threadSleepTimer) {
	...
	RakPeer::Initialize(1, clientPort, threadSleepTimer, (adapterAddress != NULL) ? vars.adapterAddress.c_str() : "\0");
...
bool RakClient::Send(RakNet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel) {
	...
	for each (Script *script in scripts) {
		if (script != nullptr) {
			if (script->luaOnSendPacket(packetData[0], packetData, packetSize))
				skipPacket = true;
		}
	}
...
Packet* RakClient::Receive(void) {
	...
	if (packet) {
		...
		/*uint8_t packetId = (uint8_t)packet->data[0];
		FILE *f = fopen("PacketLog.log", "a");
		fprintf(f, "> [Packet Recv] id: %d len: %d\r\n%s\n", packetId, packet->length, DumpMem((unsigned char *)packet->data, packet->length));
		fclose(f);*/

		bool skipPacket = false;
		for each (Script *script in scripts) {
			if (script != nullptr) {
				if (script->luaOnRecvPacket(packet->data[0], packet->data, packet->length))
					skipPacket = true;
			}
		}
...
void RakClient::PingServer(const char* host, unsigned short serverPort, unsigned short clientPort, bool onlyReplyOnAcceptingConnections) {
	RakPeer::Initialize(1, clientPort, 0, vars.adapterAddress.c_str());
...
bool RakClient::RPC(int* uniqueID, RakNet::BitStream *parameters, PacketPriority priority, PacketReliability reliability, char orderingChannel, bool shiftTimestamp, NetworkID networkID, RakNet::BitStream *replyFromTarget) {
	...
	uint8_t *packetData = parameters->GetData();
	int packetSize = parameters->GetNumberOfBytesUsed();

	bool skipPacket = false;
	for each (Script *script in scripts) {
		if (script != nullptr) {
			if (script->luaOnSendRPC(*uniqueID, packetData, packetSize))
				skipPacket = true;
		}
	}
