...
int makeKey(keyInstance *key, uint8_t direction, int keyByteLen, char *keyMaterial) {
	word8 k[MAXKC][4];
	int i;
	...
	if (keyMaterial) {
		strncpy_s(key->keyMaterial, sizeof(key->keyMaterial), keyMaterial, keyByteLen);
	} else {
		return BAD_KEY_MAT;
	}
...
int blockEncrypt(cipherInstance *cipher,
	keyInstance *key, uint8_t *input, int inputByteLen, uint8_t *outBuffer) {
	int i, k, numBlocks;
...
int cipherUpdateRounds(cipherInstance *cipher,
	keyInstance *key, uint8_t *input, int inputLen __UNUS, uint8_t *outBuffer, int rounds) {
#ifdef _MSC_VER
#pragma warning( disable : 4100 ) // warning C4100: <variable name> : unreferenced formal parameter
#endif
